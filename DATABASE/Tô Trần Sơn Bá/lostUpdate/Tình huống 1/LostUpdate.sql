...
begin tran
	begin try
		...
		if not exists (select * from DONHANG where DH_MA = @dh_ma and DH_TINHTRANG like N'Chưa xác nhân')
		begin
...
begin tran
	begin try
		...
		if not exists (select * from DONHANG with(rowlock) where DH_MA = @dh_ma and DH_TINHTRANG like N'Chưa xác thực')
		begin
			raiserror('don hang khong the xac nhan', 16, 1)
			rollback
		end
